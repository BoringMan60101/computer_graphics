...
int main()
{
    ...
    */
    ...
    // setting up data for 'line strip', that shows surface's triangles segments
    // the data is almost the same, but segments color must contrast with triangles one
    float linesColors[3 * (Nu+1) * (Nv+1)]; 
    // setting points colors in trinagles segments 
    for(int i = 0; i <= Nu; i++)
        for(int j = 0; j <= Nv; j++)
        {
            int idx = i*3*(Nv+1) + 3*j;
            //linesColors[idx + 0] = myRand(); // Red
            //linesColors[idx + 1] = myRand(); // Green
            //linesColors[idx + 2] = myRand(); // Blue
            linesColors[idx + 0] = 0.0; // Red
            linesColors[idx + 1] = 0.0; // Green
            linesColors[idx + 2] = 0.0; // Blue
        }


    // vertex arrays routine (using some data from surface's triangels arrays)
    unsigned int linesColorVBO, linesVAO;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(linesColors), &linesColors, GL_STATIC_DRAW);
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (GLvoid *)0);

    // Indicies for line segments (line strip) 
    glGenBuffers(1, &surfaceTrianglesIBO); 
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, surfaceTrianglesIBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //float angle = 0.0f;
        float angle = 0.35f * (float)glfwGetTime();
        ...
        // Send uniform variable (model matrix) to vertex shader
        ...
        // THERE IS A BUG, EXTERNAL LINES MIGHT BE DRAWN FOR SOME SURFACES
        // Drawing rulled surface's triangles with IBO indices
        glBindVertexArray(surfaceVAO);
        glLineWidth(4);
        glDrawElements(GL_TRIANGLES, sizeof(indices), GL_UNSIGNED_INT, nullptr);

        // Drawing triangle's segments with IBO indices
        glBindVertexArray(linesVAO);
    ...
    glDeleteBuffers(1, &linesColorVBO);
